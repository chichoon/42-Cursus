...
def update(request):
    try:
        if request.method == 'POST':
            form = request.POST
            data = models.Ex07.objects.filter(title=form['movie_title'])
            data.update(opening_crawl=form['movie_crawl'])
            models.Ex07.objects.get(title=form['movie_title']).save()
        lst = []
        model = models.Ex07.objects.all()
        for elem in model:
        ...
        if len(lst) == 0:
            raise Exception
        return render(request, 'ex07/update.html', {'movie_lst': lst, })
    except Exception:
        return HttpResponse("No data Available")
